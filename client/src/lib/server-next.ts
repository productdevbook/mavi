/**
 * The client boundary for the clean `server-next` API.
 *
 * `@api-next` is generated from `server-next/mavi-http/contracts/mavi.ts`.
...
const SESSION_KEY = "mavi:server-next-session"

export class ServerNextRefused extends Error {
...
export async function nextEvery<Name extends PagedOperation>(
  operation: Name,
  args: Asking<Name> = {} as Asking<Name>
): Promise<
...
> {
  ...
  for (let page = 0; page < 20; page += 1) {
    const answer = (await nextApi(operation, {
...
function refused(error: unknown): ServerNextRefused | unknown {
