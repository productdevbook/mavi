...
| `src/portable` | 403 | ~~nothing~~ `server/mavi-portable`, with a capability of its own: taking a whole site is not something being able to edit a post implies |
| `src/plugins` | 524 | **not coming back.** A site plugging into its own mail server or its own payment provider is what a port already is — the host provides one, and a table of settings for it is a second, worse way to say the same thing. See `docs/ports.md`. |
