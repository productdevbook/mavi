//! One binary, three things it can be.
//!
//! The API, the workers that take what the API queues, and the one process that
//! decides when a day's work is due. Which of them this process is comes from
//! the environment, because a machine that runs all three in one pod and a
//! machine that runs them in three should not need two builds.
