...
**Who reaches it.** Anybody signed in on a site, with no grant asked for. The
machine's own screens see every site's and write the answers.

**Tables it owns.** `reports`.

**No grant, deliberately.** What a grant would gate here is somebody telling the
people who run the machine that something is broken, and a person who cannot do
that is a person who telephones instead. Ten an hour, so a loop in a script does
not become the machine's inbox.
